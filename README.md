...
Уведомления живут внутри личного кабинета (колокольчик в шапке `/profile/`) — писем и
пушей они не шлют. Каталог типов — `Inc\Enums\Profile\NotificationType`; событийные
уведомления создаёт `NotificationSubscriber` и доменные сервисы, временны́е — крон
`NotificationCronService` раз в 15 минут. Повторы гасит ключ дедупликации: одно событие —
одна плитка на получателя. Родитель видит свои уведомления в своём кабинете.

**Ученику**

| Уведомление | Когда |
| --- | --- |
| Занятие через 30 минут | До начала занятия осталось не больше 30 минут |
| Открыт новый урок | Урок открылся: по дате занятия, вручную или сразу в открытой группе (урок-черновик не открывается) |
| Появилась запись занятия | К занятию привязали видеозапись |
| Дедлайн через 24 часа | До срока работы сутки, а сдачи нет. Срок ДЗ без явного дедлайна — начало следующего занятия |
| Дедлайн пропущен | Срок прошёл, работа не сдана (сдать её всё равно можно, сдача получит метку «Просрочено») |
| Пропущено занятие | Через 30 минут после того, как преподаватель поставил Н в журнале (с точностью до тика крона — до 45 минут). Других условий нет. Н, исправленная или снятая за эти 30 минут, не уведомляет; исправленная позже — отзывает уже пришедшее уведомление |
| Работа проверена | Преподаватель оценил работу (переоценка заменяет прежнюю непрочитанную плитку) |
| Работа возвращена на доработку | Преподаватель вернул работу |
| Экзамен проверен | Проверена попытка контрольной/экзамена |
| Попытка сброшена | Преподаватель сбросил попытки по работе или экзамену — можно пройти заново |
| Замена преподавателя | В группе утверждена замена преподавателя |
| Изменился кабинет | Разовая замена кабинета занятия или периода (смена кабинета группы на год не уведомляет) |

**Родителю**

| Уведомление | Когда |
| --- | --- |
| Дедлайн пропущен | Ребёнок не сдал работу к сроку |
| Пропущено занятие | Через 30 минут после Н в журнале — одновременно с ребёнком и по тем же правилам |
| Экзамен проверен | Проверена попытка контрольной/экзамена ребёнка |
| Ученик не посещает занятия! | Ребёнок пропустил три занятия подряд (одна плитка на серию пропусков) |

**Преподавателю**

| Уведомление | Когда |
| --- | --- |
| Занятие через 30 минут | До начала его занятия (с учётом замены) не больше 30 минут |
| Сдана домашняя работа | Ученик сдал ДЗ — одна плитка на ученика и работу |
| Сдана работа — нужна проверка | Сдана работа или контрольная с заданиями без автопроверки |
| Не заполнен журнал | Через час после конца занятия посещаемость не отмечена (открытые группы журнал не ведут) |
| Новый ученик в группе | В группу зачислен ученик — основному преподавателю группы |
| Курс обновился в КТП | Правки курса добавили или убрали уроки в программе группы — одна накопительная плитка на группу в день |
| Вам назначена замена | Преподавателя назначили на замену в чужой группе |
| Изменился кабинет | Разовая замена кабинета его занятия |

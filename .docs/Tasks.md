...
- [ ] **Убрать `$this->calendar->reflow()` из `ScheduleReflowService::pinToDate()`.** Это
...
- [ ] `pinToDate()` — найти строки группы с тем же днём (`DATE(scheduled_at)`, исключая саму
...
- [ ] **Не вытеснять `held`**: проведённое занятие — исторический факт. Если на дате стоит
...
- [ ] Продолжения темы (`continuedFromId`, T12.6) — вытеснение исходной части не должно
      осиротить вторую: решить, вытесняются обе или drop на дату части запрещён
- [ ] `src/js/profile/ktp.js` — тост после drop должен называть вытесненную тему:
      «Тема N закреплена на 1 ноября · тема M возвращена в пул»
- [ ] Тест: «drop на занятую дату — вытесненная строка обнулена, остальные даты не тронуты»
- [ ] Тест: «drop на дату с `held` отклоняется»
- [ ] Тест: «`ends_at` после `pinToDate()` не пустой»
- [ ] Регресс-тест на зонд из разбора: перетаскивание урока #5 на дату 2-го слота больше не
...
- [ ] День со слотом: время берётся из слота (`lessonTimes`), модалки нет. Если тема на дне

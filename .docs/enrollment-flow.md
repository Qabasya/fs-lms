...
Версия: 3.0 (обновлено 2026-06-06)  
Статус: реализован полностью (пути 1A–4B). Остатки: CSV-экспорт, журналы в UI, тесты.
...
Поля: номер договора, дата договора, номер приказа, дата приказа, дата зачисления, группа (числовой `id` из `fs_lms_groups`).
...
| `PersonInputDTO` | Вход для `PersonService::createOrFindBy()` (split name fields, is_student) |
| `PersonDecryptedDTO` | Выход `PersonReader::readForDisplay()` |
| `ApplicationDTO` | Строка `fs_lms_applications` (converted_record_id) |
| `StudentRecordDTO` | Строка `fs_lms_student_records` (заменяет EnrollmentDTO + ArchiveDTO) |
| `EnrollmentInputDTO` | Вход для `EnrollmentService::enroll()` (groupId — числовой int) |

...
5. В таблице logs-3-export поправить отображения Типа экспортируемых данных. Чтобы везде было читаемое название (не слаг или id из базы). Например, вместо log_entity_audit должно быть "Лог действий". При необходимости добавит 
6. Реализовать экспорт групп, учеников, родителей и архива (доделать, привязать экспорт к существующим кнопкам Экспорта)
7. Добавить в модалку application-modal.php логин и пароль ученика из формы apply (сейчас этот блок закомментирован) в открытом виде. Привести эту модалку к виду как student-person-view: сделать инпуты с границами, вместо кнопки "копировать" оставить кнопку редактирования внутри инпута.

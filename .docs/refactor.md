...
> - ✅ Фаза 4 — px → rem: ~1640 значений конвертированы (скриптовые проходы + ручные фиксы); в px осталось только осознанное: 1/1.5/2px-хайрлайны (борды), тени/фокус-кольца, 999px-пилюли, брейкпоинты — итого 422 против 2100 до. **Эквивалентность доказана**: скомпилированный CSS всех 5 бандлов после нормализации rem→px побайтово идентичен baseline (визуальных изменений ноль при дефолтном root font-size; при увеличенном шрифте браузера интерфейс теперь масштабируется — это цель). Оговорки: округление типографики к формальной шкале (14.5/12.5 → шкала) НЕ делалось (сохранён пиксель-в-пиксель); вьюпорт-эластичность контейнеров (clamp для --side-w/--colw/$max-width) перенесена в Фазу 5 — это дизайн-решение, требует глаз
> - ✅ Фаза 5 (v1, add-only) — адаптив: ТОЛЬКО добавленные @media-правила, десктоп не тронут. frontend: формы apply/join/auth (компактные карточки), табы со скроллом, контрольная (кнопки в столбец). profile: сайдбар — off-canvas на ≤720 (поверх контента, дефолт скрыт — app.js initCollapse), плитки дашборда в столбец, недельная сетка и календарь КТП со скроллом, банк тем над календарём, компактный журнал (сетка и так скроллится со sticky-колонками), форма инд. занятия без min-width 500. player: лента шагов со скроллом, сопоставление в столбец, воркбар/баннер результата wrap (рейка на ≤640 скрыта как и была; на планшетах — pin по клику). Все magic numbers в @media заменены токенами ($bp-*/$breakpoint-*; group-cockpit 768→$bp-wp — сдвиг порога на 14px). **Нужна визуальная проверка на 360/768px**; вьюпорт-эластичность контейнеров (clamp) и touch-размеры (44px) — кандидаты v2 по итогам проверки
> - ✅ Фаза 7 (2026-07-31, аудит `Tasks.md` §1.4) — **одна лестница токенов на весь проект**: имя ступени теперь всегда значит один и тот же размер/вес. Веса `$font-regular/$font-semibold/$font-bold` (было: `$font-bold` = 600 в ядре и 700 во frontend); кегли `2xs 10 / code 11 / xs 12 / sm 13 / base 14 / md 16 / lg 22 / xl 24 / 2xl 28`; отступы `xs 4 / sm 8 / md 12 / lg 16 / xl 20 / 2xl 24 / 3xl 28 / 4xl 32`; радиусы `sm 4 / md 6 / lg 8 / xl 12 / 2xl 16 / pill 999px`. `frontend/_variables` больше не переопределяет общие ступени, а форвардит их из `shared/tokens`. Одноимённые, но разные по смыслу токены разведены: `$font-code` (моноширинный публичных страниц) vs `$font-mono` (кабинет/плеер), `$shadow-surface` vs `$shadow-card`, `$line-height-relaxed` (1.6) vs `$line-height-base` (1.5). Введён слой сырых оттенков (`$hue-violet`, `$hue-violet-dk`, `$hue-red`, `$hue-red-soft`, `$hue-amber-dk`) — один физический цвет объявлен один раз, палитры ссылаются на него. **Эквивалентность доказана**: CSS всех 7 бандлов побайтово идентичен baseline на каждом шаге переименований.
>   Осознанные визуальные микро-правки (в том же проходе, аудит просил схлопнуть почти-дубли): ступень радиуса 3px удалена (→ 4px, только admin); `$cb-chip-bg`/`$cb-badge-bg` → `$wp-admin-gray-2`; `$table-header-bg` → `$wp-admin-gray-bg-light`; `--good-bg`/`--good-bg-soft` → `--ok-soft`/`--ok-soft-2`; `--wait-ink` → `var(--wait)`. Оставлены намеренно: `--line`/`--line-2`, `--accent-soft`/`--accent-soft-3`, `--surface-2`, `$cb-bg-tree` — это иерархия поверхностей, а не дрейф.
> - 🟡 Фаза 6 — частично: `_modal.scss` (1152 строки) распилен на 11 партиалов `components/modal/*` (base/forms/utilities + 7 фич-модалок + mixins), порядок каскада сохранён 1:1 — собранный CSS побайтово идентичен. Admin-тост уже был реэкспортом common — дубля нет. **Не сделано**: снижение 114 `!important` (модалка воюет с WP-core/WPBakery; вычистка требует ручной визуальной проверки каждой модалки в админке — механически недоказуемо). Все `!important` видны как warnings в `lint:css` — рабочий список на будущее
>

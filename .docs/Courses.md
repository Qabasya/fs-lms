...
## 11. Разделение ролей: курс — методисту, занятие — преподавателю

Справка по механике — `basic_doc.md` §35 («Форк урока для группы», «Teacher-режим плеера»,
«Шаг „Трансляция"»). Парадигма:

1. **Курс составляет только методист в админке** — задачи, уроки, шаги, работы и контрольные.
   У преподавателя нет ни редактора шагов, ни авторинга банка (капа `AuthorLmsBank` откачена).
2. **Преподаватель проводит занятия**: отмечает посещаемость и открывает урок своей группы
   в teacher-режиме плеера (`?gid&gl`) для демонстрации — без прогресса, с «Показать решение».
   Отдельно, в разделе «Мои курсы» ЛК, он может пройти курс «от роли ученика» в preview-плеере
   (`/course-preview/?course=`): ответы проверяются dry-run, ничего не сохраняется. Методисту
   тот же предпросмотр открывает кнопка «Просмотр» в курс-билдере.
3. **КТП уроки не меняет** — расписание/размещение тем, но не состав шагов.
4. **Шаг `broadcast`** заменил чекбокс `recording_slot` у видео-шага (миграция идемпотентна,
   `key` шага сохранён). Запись подтягивается авто-матчем VideoLibrary; не привязалась —
   алёрт в админке (блок «Занятия без записи» + `admin_notices`) с привязкой в один клик,
   либо попап камеры в КТП группы для ручной ссылки.

Форк урока под группу (`forkLessonForGroup`, меты `ForkedFrom`/`ForkedForGroup`) остаётся
инструментом методиста; удаление группы чистит форки (`GroupDeletionHandler` →
`deleteForksForGroup`).

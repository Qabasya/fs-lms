...
- **`{key}_works`** — работа (типизированный пул заданий). `post_title` = название работы. Мета через `PostMetaName::Meta`:
  - `work_type` — `practice | independent | homework` (enum `WorkType`)
  - `task_ids[]` — **упорядоченные ссылки** на `{key}_tasks` (`PostTypeResolver::tasks($key)`), не копии

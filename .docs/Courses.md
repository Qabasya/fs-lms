...
## 11. Урок группы как снапшот курса (COW-форк) + шаг «Трансляция» — реализовано

Полный план и статус — `.docs/Tasks.md`; справка по механике — `basic_doc.md` §35
(«COW-форк урока группы», «Шаг „Трансляция"»). Кратко:

- **Шаг `broadcast`** заменил чекбокс `recording_slot` у видео-шага (миграция идемпотентна,
  `key` шага сохранён). Запись занятия подменяет заглушку через фильтр `fs_lms_recording_url`.
- **COW-форк**: первая правка шагов преподавателем (`SaveGroupLessonSteps`) форкает мастер
  (`ContentCloneService::forkLessonForGroup`, меты `ForkedFrom`/`ForkedForGroup`); до этого
  группа видит мастер и правки методиста доезжают. Клиент оперирует только `group_lesson_id`.
- **Teacher-режим плеера** (`?gid&gl`, `LessonPlayerController`): просмотр без прогресса
  ученика + редактор шагов (`teacher-editor.min.js`, read-only банк, тич-панель записи
  в broadcast-шаге).
- **Гигиена форков**: бейдж «изменён» в КТП (`is_forked` из `ScheduleService::getCalendar`),
  «Сбросить к версии курса» (`resetLessonFork` — прогресс по добавленным преподавателем
  шагам теряется, конфирм озвучивает), удаление группы чистит форки
  (`GroupDeletionHandler` → `deleteForksForGroup`).

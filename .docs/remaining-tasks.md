...
* Редактировать (вход в режим редактирования, все данные становятся инпутами, логгирование)
* Экпорт ПД
* Удалить ПД (и пользователя). Требуется дополнительное окно подтверждения confirm-modal

---

## 1. Шаблоны — заглушки и отсутствующие

### 1.1 `templates/admin/enrollment/person-detail.php`

Карточка person. Рендерится из `PiiCallbacks::renderPersonDetailPage()`. Шаблон один, вид определяется ролью: в PHP получаем `PersonDTO::wpUserId` → `get_userdata` → проверяем роль WP-пользователя (`lms_student` / `lms_parent`), и на основе этого условно показываем нужные вкладки.

#### Вид карточки для ученика (`lms_student`):

Три вкладки:
- **Данные** — PII с масками + кнопки «Показать» → `ajaxRevealPiiField`; открытие маски логируется в `PiiAccessLogRepository`; кнопки «Редактировать», «Удалить ПД», «Экспорт ПД»
- **Представители** — таблица `RelationshipRepository::findActiveByStudent()` + кнопки «Добавить представителя», «Заменить»
- **Зачисления** — таблица `EnrollmentRepository::findByStudent()` (история: группа, период, статус, дата)

Модалки (в том же шаблоне):
- «Добавить представителя» — форма нового guardian → `ajaxAddRepresentative`
- «Заменить представителя» — выбор старой связи + форма нового → `ajaxReplaceRepresentative`
- «Редактировать данные» — форма обновления PII → `ajaxUpdatePerson`

#### Вид карточки для родителя (`lms_parent`):

Три вкладки:
- **Данные** — PII с масками + кнопки «Показать» → `ajaxRevealPiiField`; открытие маски логируется в `PiiAccessLogRepository`; кнопки «Редактировать», «Удалить ПД», «Экспорт ПД»
- **Подопечные** — таблица `RelationshipRepository::findActiveByGuardian()`: ФИО ученика, дата начала связи, тип отношения; ссылка «Открыть карточку» → карточка ученика
- **Зачисления** — сводная таблица зачислений всех подопечных: через связи получаем `student_person_id` каждого ребёнка, затем `EnrollmentRepository::findActiveByStudent()` для каждого
...
### 1.3 `templates/admin/components/tabs/userlist-tabs/userlist-5-archive.php`

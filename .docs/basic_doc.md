...
>
> Состояние: релиз 1.0.8 (что изменилось относительно 1.0.7 — §43). Детальные спеки соседних
...
**Dry-run** (чекбокс «Только проверить») — полный проход файла без записи: заголовки
валидируются, каждая строка резолвится (группа, persons, дедуп `existsByContract`), но
перед записью импортёр возвращает результат с `label` — «ФИО — группа «…», договор № …»
(+ «, в архив» / «, логин …»). При `dryRun` `ImportReportDTO` копит их в `$preview`
(`[label, status, note]`), клиент рендерит нумерованный список «Будут добавлены следующие
архивные записи / Будут зачислены следующие ученики» и отдельно — что будет пропущено.
В боевом запуске `preview` пуст. Ни записи, ни учёток, ни писем, ни лог-событий dry-run не создаёт.

Набор колонок — `ImportColumn::headers()`: **шаблон один на оба режима** (все колонки),
режимные колонки в «чужом» режиме не читаются и могут быть пустыми (`Логин`/`Пароль` — в
архиве, колонки отчисления — при полном зачислении). Обязательность значений — режимная,
`ImportColumn::required( ImportMode )`.
...
`NewRepresentative`, `WelcomeWithCredentials`, `CourseGranted`). Плейсхолдеры в options-шаблонах — `{key}`.
...
> **В админке реально редактируются только 2 из 8 типов** (`otp_code`,
> `welcome_with_credentials`) — массив `$types` в
> `settings-4-email-templates.php` захардкожен, не строится по `EmailTemplateType::cases()`.
> `CourseGranted` реально отправляется, но не имеет UI-редактора (уходит из PHP-файла as is).
> Остальные 5 — не вызываются из кода (заготовки на будущее). Решение (2026-07-23):
> оставшиеся типы признаны ненужными, UI под них не расширяем.
>
> **`welcome_with_credentials` уходит ТОЛЬКО родителю** (решение 2026-07-23).
> `EnrollmentService::enroll()` раньше слал это письмо и ученику, и родителю с одинаковым
> `$sharedVars` — но новый дизайн письма обращается по имени родителя и пишет «ваш ребёнок
> зачислен», что некорректно для письма самому ученику. Строка отправки ученику удалена;
> `sendWelcomeWithCredentials()` теперь вызывается один раз (только `$guardianUserId`).
> Логин/пароль ученика по-прежнему создаются и возвращаются в `EnrollmentResultDTO` —
> office видит их в карточке зачисления и передаёт при необходимости сам.

Новый тип письма: PHP-файл шаблона → кейс в `EmailTemplateType` → метод в `EmailService`.

### Что можно/нельзя вставить в поле «Текст письма (HTML)» админки

Поле — `<textarea>`, при сохранении (`EmailTemplateSettingsCallbacks::ajaxSaveEmailTemplate`)
проходит `Sanitizer::sanitizeHtml()` → `wp_kses_post()` (тот же whitelist, что у контента
постов). Из этого следует:

- **Можно**: HTML-фрагмент — `<table>`, `<tr>`, `<td>`, `<span>`, `<p>`, `<a>`, `<img>` и т.д.
  с инлайновыми атрибутами `style="..."` — они разрешены в `wp_kses_post()`.
- **Нельзя**: полный HTML-документ. `<!DOCTYPE>`, `<html>`, `<head>`, `<body>`, `<meta>`,
  `<script>` — не в whitelist, вырезаются целиком. Тег `<style>` тоже вырезается, но текст
  CSS-правил внутри него **остаётся как видимый plain-текст** в письме (не удаляется вместе
  с тегом) — главный источник «поломанной» вёрстки при вставке экспорта из дизайн-тулов.
- Кастомные теги дизайн-инструментов (`<x-dc>`, `<helmet>` и т.п.) тоже не в whitelist —
  срезаются как неизвестные.
- Обёртки/layout вокруг сохранённого body нет — что сохранено, то и уходит в письмо
  (`Content-Type: text/html` уже выставлен в `EmailService`).
- **Практика**: из экспорта дизайн-тула нужно брать только внутренний фрагмент вёрстки
  (div/table с inline-стилями), без DOCTYPE/html/head/script/кастомных тегов и без
  `<style>`-блока (стили в дизайн-экспортах обычно и так продублированы инлайново).
- Для дефолтного (не переопределённого через админку) письма ограничений `wp_kses_post` нет —
  `templates/emails/{type}.php` возвращает body напрямую в `wp_mail()`, без прогона через
  kses. Поэтому полноценную HTML-вёрстку писем разумнее держать как PHP-дефолт, а не
  вставлять через админ-поле.
...
Релиз 1.0.8 (2026-09-15, тег `v1.0.8`) — всё, что сделано после «Версия 1.0.7» (`4463e79b`,
2026-09-13). Разборы первопричин, замеры и

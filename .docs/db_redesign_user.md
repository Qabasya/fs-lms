...
**Что хранится:** ФИО человека (раздельными полями), дата рождения (для более точной идентификации), его роль (булево значение: True для ученика, False для родителя), адрес его профиля wp_user_id и даты. Через ключ id можем получать данные из других таблиц.Для передачи используется DTO
...
Иметь возможность перевести запись из student_records обратно в Заявки (кнопка "Сформировать заявку") для перезачисления студента (как при отчислении, так и без отчисления, например, на еще одно направление). Продумать универсальность.
...
docker exec wp_db mariadb -u root -proot wordpress -e "                                                                                                                                                                                                                                                          
  ...
  DROP TABLE IF EXISTS wp_fs_lms_auth_log;           
  DROP TABLE IF EXISTS wp_fs_lms_consents;  
  DROP TABLE IF EXISTS wp_fs_lms_consent_change_log;       
  DROP TABLE IF EXISTS wp_fs_lms_entity_audit_log;      
  DROP TABLE IF EXISTS wp_fs_lms_data_change_log;                            
  DROP TABLE IF EXISTS wp_fs_lms_deletion_log;      
  DROP TABLE IF EXISTS wp_fs_lms_email_log;              
  DROP TABLE IF EXISTS wp_fs_lms_export_log;                                                                                                                                                                                                                            

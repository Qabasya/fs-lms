<?php
/**
 * Разметка письма с OTP-кодом — ЕДИНСТВЕННЫЙ источник.
 *
 * Возвращает HTML с плейсхолдером `{code}`: письмо (`templates/emails/otp_code.php`)
 * подставляет значение, вкладка «Шаблоны писем» показывает эту же разметку
 * как заготовку (раньше она была скопирована туда heredoc'ом — аудит §P3).
 *
 * @package FS LMS
 */

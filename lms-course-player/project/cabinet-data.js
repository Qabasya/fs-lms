/* ══════════════════════════════════════════════════════════════════════
   Данные кабинета (Информатика). Детерминированная генерация заполнения.
   ══════════════════════════════════════════════════════════════════════ */

// seeded RNG so the filled journal is stable across reloads
...
const AVA_COLORS = ['#5c7cfa','#7048e8','#1c7ed6','#0ca678','#f08c00','#e8590c','#e64980','#9c36b5','#2f9e44','#4263eb','#0c8599','#f76707'];
...
// journal columns for «2 четверть»
const J_COLUMNS = [
...
// build student rows with deterministic attendance + grades
function buildJournal() {
  const students = STUDENT_NAMES.map((name, si) => {
    const rnd = mulberry32(1000 + si * 97);
    const strength = 0.55 + rnd() * 0.45; // student "ability" 0.55..1
    const attBias = 0.86 + rnd() * 0.12;
    const cells = J_COLUMNS.map((col, ci) => {
      const r = rnd();
      // attendance
      let att = 'p';
      if (r > attBias) att = 'n';
      ...
      if (att !== 'n') {
        if (col.type) {
          // works almost always graded
          if (rnd() < 0.94) grade = gradeFrom(strength, rnd, col.type);
        } else {
          // plain lessons: occasional oral grade
...
function gradeFrom(strength, rnd, type) {
  // harder works pull grades down a touch
...
// ── Dashboard: расписание на сегодня ─────────────────────────────────
const TODAY_LESSONS = [
...
// ── КТП ──────────────────────────────────────────────────────────────
const KTP_CONFIG = {
  year: 2025, month: 10, // Ноябрь (0-indexed)
  lessonDows: [2, 5],    // Вт, Пт
  holidays: [{ day: 4, name: 'День народного единства' }],
  // pins: themeId -> day-of-month (фиксированные темы)
  pins: { t1: 7 },
};
...
Object.assign(window, {
  WORK_TYPES, GROUPS, JOURNAL, J_COLUMNS, avgOf,
  TODAY_LESSONS, WORKLIST_ATT, WORKLIST_REV,
  KTP_CONFIG, KTP_THEMES, MONTHS_RU, DOW_RU,
});

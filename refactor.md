...
> Статус: **план, не выполнено**. Порядок фаз = порядок исполнения; каждая фаза — отдельный коммит/PR, после каждой — `npx gulp styles:check` + визуальная проверка затронутого бандла.

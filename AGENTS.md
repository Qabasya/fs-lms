...
распаковки описаны в `inc/Services/Subject/Bundle/AGENTS.md` (грузится при работе с каталогом).
...
SVG-иконки) — `src/js/AGENTS.md`, грузится при работе с `src/js/`.
...
- **Номер задания (слаг) выдаёт `TaskNumberService`**: `{номер задания}{счётчик из 3 цифр}`,
  наименьший свободный — в отличие от статей, дыры переиспользуются (удалённое 5001 займёт
  следующее задание). Модалка и перенос легаси-заданий идут через него же
  (`TaskManager::createNewTask()`). Выдача не атомарна, поэтому до первой публикации
  `SubjectValidationCallbacks` проверяет, что номер принадлежит номеру задания и не занят;
  у уже опубликованных заданий номер не перепроверяется
...
`src/scss/AGENTS.md`, грузится при работе с `src/scss/`.
...
> **Codex может выполнять docker-команды напрямую** (перезапуск контейнера, запросы к БД, прогон миграций, проверка рантайма). Сервисы: `wp_app` (WordPress:8080), `wp_db` (MariaDB), `wp_phpmyadmin` (phpMyAdmin:8081).

...
class ContentCloneServiceTest extends TestCase {
	...
	public function test_delete_forks_for_group_deletes_only_this_groups_forks(): void {
		...
		$this->seedMasterAndFork();
		fs_test_seed_post(
			array( 'ID' => 3, 'post_type' => 'inf_lessons', 'post_title' => 'Чужой форк', 'post_status' => 'publish' ),
			array(
				PostMetaName::Meta->value           => array( 'steps' => array() ),
				PostMetaName::ForkedFrom->value     => 1,
				PostMetaName::ForkedForGroup->value => 20,
			)
		);
		// listByGroup(10) отдаёт строки: мастер-занятие и форк-занятие. Строка с чужим
		// форком (id=3) в группе 10 невозможна by design — не сидируем.
		$this->wpdb->queueResults( array(
			array( 'id' => 5, 'group_id' => 10, 'lesson_id' => 1, 'position' => 1, 'extra_work_ids' => '[]', 'visibility' => 'open', 'allow_late' => 1 ),
			array( 'id' => 6, 'group_id' => 10, 'lesson_id' => 2, 'position' => 2, 'extra_work_ids' => '[]', 'visibility' => 'open', 'allow_late' => 1 ),
		) );

		self::assertSame( 1, $this->service->deleteForksForGroup( 10 ) );

		self::assertNotNull( $this->lessons->get( 1 ) ); // мастер жив
		self::assertNull( $this->lessons->get( 2 ) );    // форк группы удалён
		self::assertNotNull( $this->lessons->get( 3 ) ); // чужой форк не тронут
	}
}

...
export const PersonDetail = {
    ...
    _bindAddRepresentativeModal() {
        ...
        $modal.find( 'form' ).on( 'submit', ( e ) => {
            ...
            $.post(
                fs_lms_vars.ajaxurl,
                formData + '&action=' + fs_lms_vars.ajax_actions.addRepresentative + '&security=' + fs_lms_person_vars.nonces.add_representative
            ).done( ( res ) => {
                if ( res.success ) location.reload();
            } );
        } );
    ...
    _bindReplaceRepresentativeModal() {
        ...
        $( document ).on( 'click', '.js-open-replace-representative', ( e ) => {
            ...
            $modal.find( '[name="archive_id"]' ).val( archiveId );
        ...
        $modal.find( 'form' ).on( 'submit', ( e ) => {
            ...
            $.post(
                fs_lms_vars.ajaxurl,
                formData + '&action=' + fs_lms_vars.ajax_actions.replaceRepresentative + '&security=' + fs_lms_person_vars.nonces.replace_representative
            ).done( ( res ) => {
                if ( res.success ) location.reload();
            } );
        } );

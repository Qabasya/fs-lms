...
- **Картинки из WPBakery — третий способ.** В статьях вложение лежит атрибутом
  шорткода (`[vc_single_image image="143"]`, `images="1,2,3"` у галерей) — без
  класса и URL. Карту «шорткод → атрибуты» держит `ShortcodeMediaRefs`
  (элементы WPBakery по умолчанию, модули добавляют свои фильтром
  `fs_lms_bundle_shortcode_media_attrs`); экспорт берёт оттуда ID, импорт
  меняет их по карте `MediaUrlRewriter::buildIdMap()` (каждый ID — один раз).

...
class SubjectSettingsCallbacks extends BaseController
{
	...
	public function ajaxGetTasksByNumber(): void
	{
		...
		// Запрос постов с указанным термином
	...
	public function ajaxGetRecentTasks(): void
	{
		...
		// Рендерим шаблон
		ob_start();
		$this->render('components/ajax-tables/tasks-ajax-table', [
			'rows'       => $rows,
	...
	public function ajaxGetRecentArticles(): void
	{
		...
		ob_start();
		$this->render('components/ajax-tables/articles-ajax-table', [
			'rows'       => $rows,
			'taxonomies' => [],
		]);
		$html = ob_get_clean();

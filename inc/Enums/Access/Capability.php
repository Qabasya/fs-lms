...
// ===== Администрирование =====
	...
	/**
	 * Право работы с разделом «Предметы»: таксономии предмета, типовые условия
	 * и шаблоны заданий, CRUD самого предмета, импорт/экспорт и перенос пакетом.
	 *
	 * Отдельное право, а не `Admin`/`ManageLmsPlatform`: раздел наполняет
	 * методист, которому нечего делать в остальной платформе (заявки, ПД,
	 * зачисление), и не должен требовать `manage_options` — с ним методист
	 * получил бы всю админку WordPress.
	 */
	case ManageSubjects = 'manage_lms_subjects';
